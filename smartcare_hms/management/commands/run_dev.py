...
class Command(BaseCommand):
    ...
    def _verify_redis_connection(self):
        """Check if Redis is accessible before starting Celery."""
        try:
            import redis
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        '  Celery worker will fail to start. Ensure Redis is running.\n'
                    )
                )
                raise
    ...
    def _start_celery_worker(self):
        ...
        try:
            ...
            if process.poll() is None:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        '  ✗ Celery worker failed to start. Check Redis connection and CELERY_BROKER_URL.'
                    )
                )
            ...
            self.stdout.write(
                self.style.ERROR(
                    '  ✗ Celery not installed. Run: pip install celery[redis]'
                )
            )
            raise
